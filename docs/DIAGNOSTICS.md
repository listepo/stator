...
| STA1210 | both | not-yet | {member} is not yet supported; planned for Phase 4 (builtins) | 4 | `Date`'s RESIDUE code, the shape `STA1211` has for RegExp: slice A landed 2026-09-01 (plan-notes 130) and this code now names one MEMBER at a time rather than the class. What is left under it: the 8 local getters, `getTimezoneOffset`, the 7 local setters and the component constructor `new Date(y, m, …)` -- slice B, blocked on pinning `TZ` in the golden runner, which is Phase 4's own step 8; and `toString`/`toDateString`/`toTimeString`/`toLocale*`, whose long timezone and locale names are ICU CLDR data and therefore join Task 4.4's feature build (the `STA1215` model). `Date.now()` and zero-argument `new Date()` are ACCEPTED, not deferred: nondeterminism is a proof problem, not an acceptance problem, and they prove through the determinism carve-out (`tests/unit/date-clock.test.ts`). Per §15's rule, when Phase 4 closes every member still under this code must be delivered or reassigned |
| STA1211 | both | not-yet | RegExp methods are not yet supported; planned for Phase 4 (builtins) | 4 | Stays Phase 4. The array-with-properties blocker is CLOSED: `JSRTArray` carries the dynamic object's property table, and `test` (Task 4.3), `exec` and `String.prototype.match` (Task 4.1) have all landed. What remains under this code is the DATA PROPERTY surface -- `source`, `flags`, `lastIndex`, `global`, `dotAll`, … -- plus `toString` and `compile`, which need reads the object model has no node for; that is Task 4.2's remaining RegExp work. `String.prototype.matchAll` SPLITS OFF to Phase 5 step 8 -- it answers an iterator (plan-notes 116) |
...
| STA1215 | both | not-yet | String.prototype.{op} needs the ICU feature build: rebuild with `make -C runtime intl` and compile with STATOR_RUNTIME=intl | 4 | `localeCompare`, `toLocaleLowerCase`, `toLocaleUpperCase`. Not a scheduling deferral like the rest of this band and not permanent either: the code is written and the CHECK is a build configuration, so the message names the flag that turns it on rather than a phase. Collation and tailored casing are CLDR data (~10 MB), which the default runtime does not carry (Phase 4 Task 4.4) |
