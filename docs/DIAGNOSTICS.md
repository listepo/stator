...
| STA2008 | both | error | extern '{name}' is only legal in a script (non-module) .d.ts file included with /// <reference path>; {detail} | Task 7.1 surface (docs/FFI.md §1). A `.ts` ambient keeps the gate's overload-signature `STA1214`; a marked extern in a module `.d.ts` or any other file is this error. Keeping externs in declaration files is what makes Task 7.3's generator output a drop-in |
| STA2009 | both | error | malformed '@stator{tag}' marker on extern '{name}'; {detail} | Task 7.1 surface (docs/FFI.md §2). Misplaced marker, duplicate single-valued tag, unknown `@stator*` tag, C symbol outside `[A-Za-z_][A-Za-z0-9_]*`, `@statorAbi` naming no parameter, or `@statorThrows` outside the closed set |
| STA2010 | both | error | extern '{name}' uses type '{t}', which has no C mapping; see docs/FFI.md §3 | Task 7.1 surface (docs/FFI.md §3). `Unknown`, objects, arrays, and closures are refused by construction — they would need boxing — as is a bare `string` with no `const char*` selection: UTF-16 in means bytes out, a real conversion that is spelled at the declaration and never inferred |
| STA2011 | both | error | extern '{name}' must be a plain function: no type parameters, no overloads, no optional or rest parameters, no 'this' | Task 7.1 surface (docs/FFI.md §3). A C prototype has no `undefined` to pad with, no varargs in v0, and no body for overloads to share |
...
- **STA2012–STA2999**: Lowering and boundary errors (Phase 2 onward)
