...
| `STA2xxx` | Lowering / boundary errors | error, runtime | Cast failures and unimplemented lowering operations are compile-time `error`; boundary-check failures are class `runtime`, raised by the emitted program rather than the compiler (`STA2001`, `STA2002`, `STA2004`, `STA2006` abort; `STA2008`–`STA2009` throw a catchable `TypeError` instead) |
...
| STA1002 | ts | never | .js files are not allowed in ts mode; use `--mode=js` or convert to .ts | Attempt to load a `.js` file when mode is `ts`. Hint directs user to the right mode. `allowJs` is on in both modes so tsc keeps the file; the gate is what refuses it (plan-notes 141) |
...
| STA1119 | both | never | {kind} in an extern signature is outside the ABI table (docs/FFI.md) | Extern catch-all: struct by value, `void` as a parameter, `CStringOwned` as a return, a `void` return combined with a value-signalling error convention, a malformed `@statorLink` pragma (or one in a file with no extern declaration, or a second `#include`), and anything else the table does not name. (What the gate cannot see it does not claim: whether a `void`-mapped C symbol returns a value is unknowable from the declaration alone, so only the convention combination is refused.) A future task that widens the table splits a kind out with a NEW code, never by reusing this one |
...
| STA1122 | both | never | exported {kind} '{name}' cannot be exposed to C in v0 — {reason} (docs/FFI.md) | Export refusal (plan §10 Task 7.2 step 2): a class, a closure-valued const, a generic function, a rest/destructured parameter, an `export default` or re-export form, or any other exported shape that is not a non-generic function declaration. The kind names the shape; every variant is permanent for v0, so one code (unlike the extern table, whose kinds may split out as the table widens) |
| STA1123 | both | never | exported '{name}' is mutable or non-primitive state and cannot be exposed to C in v0 — {reason} (docs/FFI.md) | Export refusal (plan §10 Task 7.2 step 2): `let`/`var` exports, destructured const lists, uninitialized consts, and consts of non-primitive type. Only `const` number/boolean (C type), `CString` (`const char *`), and string/null/undefined (`jsrt_value`) are exportable in v0 |
| STA1125 | both | never | an `Out<T>` out-slot is used outside its contract (docs/FFI.md) | Extern refusal: `Out<T>` needs a branded-pointer inner, lives only in locals, passes only to `Out<T>` parameters, and reads only through `.value`. Anything else — a non-brand inner, a `T**` spelled any other way, an `Out` flowing into a return, an object/array, a non-`Out` binding, or a dynamically-typed argument where only a proven slot is sound (an out-param WRITES through the pointer, so unlike a `T*` read it cannot trust bits) — is this code. Split out of STA1119 when the table widened to `T**` (docs/FFI.md §2) |
| STA1126 | both | never | exported '{name}' holds an `Out<T>` out-slot and cannot be exposed to C in v0 — out-slots are call-local rooted addresses (docs/FFI.md) | Export refusal (plan §10 Task 7.2 step 2): an `Out<T>` value names a frame-local slot whose address dies with the call, so it has no C-observable meaning across the boundary |
| STA1130 | both | never | the generator cannot express {kind} from {header}:{line} (docs/FFI.md) | Generator scope refusal (plan §10 Task 7.3 step 5): macros/macro constants, enums, inline functions, globals, unions, and bitfields are rejected with construct + header line rather than silently skipped. Refusals that have a gate equivalent (out-params, int64, function pointers, varargs, …) cite the gate code instead |
| STA1124 | both | never | exported '{a}' collides with exported '{b}' on the C symbol '{symbol}' — rename one (docs/FFI.md) | Export refusal (plan §10 Task 7.2 step 7): two exports mangling to one `stator_<unit>_<name>` is a compile error, never last-writer-wins |
...
|---|---|---|---|---|---|
| STA1201 | both | not-yet | generators are not yet supported; planned for Phase 5 (the iterator protocol) | 5 | `function*` declarations and unnamed expressions landed with step 8 (plan-notes 152). The code now names the residue: generator methods, async generators, and `for await`. `yield*` is STA1214. User-iterable `for-of` landed (plan-notes 154) |
...
| STA1205 | js | not-yet | delete on a statically-shaped object is not yet supported in js mode; planned for Phase 8 (dynamic tier) | 8 | **NARROWED by plan §8 step 2a(c)** (was "delete on properties"): the operator itself landed. On a dynamic-shape receiver `delete` is `jsrt_delete`, which answers the boolean by REBUILDING the object's shape chain from the root without the key — shapes are shared immortal metadata, so a node cannot be dropped in place, and replaying transitions lands two objects that deleted the same key on one shape. What still defers is a receiver whose shape is FIXED at compile time, which has no encoding for a missing slot; it lifts with the same dictionary-mode escape STA2004 names. ts mode refuses that case permanently (STA1108); an Unknown receiver that turns out fixed at run time aborts with STA2007 |
| STA1206 | js | not-yet | eval() and new Function() are not yet supported in js mode; planned for Phase 8 (dynamic tier) | 8 | One code for both dynamic-code-generation constructs — they land together with the Phase-8 interpreter tier. ts mode rejects them permanently (STA1101, STA1103). Emitted since Phase 5 step 2; the interpreter that retires it is Phase 8 |
| STA1207 | both | not-yet | import() with a computed specifier is not yet supported; planned for Phase 8 (runtime module resolution) | 8 | Literal-specifier `import()` landed in Phase 5 step 10 (plan-notes 156): a namespace `HObject` whose field reads are the export globals, wrapped in `Promise.resolve`. The code stays allocated for the computed-specifier half and for a non-arity-1 call. Never reuse. |
| STA1208 | both | not-yet | top-level await is not yet supported: a module body has no resume point to suspend into | 5 | **No longer emitted** (Phase 5 step 9, plan-notes 155). The module body is an async unit. Kept allocated so the number is never reused. |
| STA1210 | both | not-yet | {member} needs the ICU feature build: rebuild with `just runtime-intl` and compile with STATOR_RUNTIME=intl · {member} is not yet supported; planned for Phase 5 | none / 5 | `Date`'s RESIDUE code, the shape `STA1211` has for RegExp: it names one MEMBER at a time rather than the class. Slice A landed 2026-09-01 (plan-notes 132) and slice B the same day (plan-notes 133), which leaves exactly the ICU-dependent string forms: `toString` and `toTimeString`, whose output appends the host zone's LONG display name (`(Central European Summer Time)`) that Node reads from ICU and libc's `%Z` cannot produce, and the three `toLocale*`. **Those carry NO PHASE** (2026-09-01, plan-notes 136): their blocker is the FEATURE BUILD, exactly the `STA1215` model, and a phase number would tell the user to wait for something already shipped. The code's other sites -- arity and spread refusals on `Date`/`Date.prototype`/`new Date` -- are ordinary lowering work and name **Phase 5**, since Phase 4 owns nothing further here. `toDateString` is NOT among the residue: its output carries no zone name, so it landed with slice B. `Date.now()` and zero-argument `new Date()` are ACCEPTED, not deferred: nondeterminism is a proof problem, not an acceptance problem, and they prove through the determinism carve-out (`tests/unit/date-clock.test.ts`) |
...
| STA1212 | both | not-yet | Symbol is not yet supported; planned for Phase 5 | 5 | Well-known symbols as *values*, `Symbol.for`/`Symbol.keyFor`, and the global registry. Emitted for `Symbol(...)` and `Symbol.iterator` as a stored value. `[Symbol.iterator]()` as a class method name is admitted (plan-notes 154) |
...
| STA1217 | both | not-yet | using an extern function as a value is not yet supported (v0 has no C value representation for externs) | none | **The FFI surface code** (plan §10 Task 7.1 steps 1–2): emitted for `@statorExtern`-marked declarations the lowering does not cover. Since Phase 7 closed that is the extern-as-value position only — branded-pointer (`T*`) signatures compile borrow-only since step 6, header/link plumbing landed in step 7, and the optional call delivered at close-out (`?.` on an always-linked callee is a proven no-op) — never the call itself. No open phase owns a C value representation, so the code carries no phase (the `src/support/phases.ts` no-phase case). The eight `STA1114`–`STA1121` refusals are `never` codes in the `STA11xx` table — design limits of the v0 surface, not schedule — so the ranges stay disjoint per §1.3 |
...
| STA2001 | both | runtime | boundary check failed at {file}:{line}:{col} — expected {expected}, got {actual} | **The first runtime-emitted diagnostic**, and since Task 3.5 a live one. Raised by `jsrt_check_number/string/boolean` when a value crossing a type boundary does not match the type the program claimed. Spellings that reach it: a read of an `unknown` at a point the checker narrowed it (a `typeof` guard, an `instanceof`, an `!== undefined`); an `as` cast off an `unknown`; and a dynamic value reaching an annotated binding, parameter, or return — the mixed-graph edge (Phase 5 step 5). `JSON.parse` lands `unknown` and uses the same wrap at those sites. Not a compile error by construction: the boundary exists precisely because the type is unknowable statically (plan §0.2). The location is exact rather than reconstructed — the emitter bakes `file:line:col` in as a string literal, because it is the last party that still knows it. Failure ABORTS: everything emitted downstream of a check is entitled to trust the type completely, and a check that could be ignored would make that false |
| STA2004 | both | runtime | a statically-shaped object cannot grow a new property; planned for Phase 8 | **The third runtime-emitted diagnostic.** Raised by `jsrt_set_prop` when a fixed-layout `JSRTObject` is asked to grow a key its `JSRTClass` descriptor does not list. Reads and writes of *existing* fields walk the descriptor (Phase 5 step 4), so `const a = { x: 1 }; const b: { x?: number } = a; b.x` prints `1`. `b.y = 2` against a layout that has no `y` still cannot invent a slot, and aborts rather than silently dropping the write. Not a compile error: whether a given value arrives fixed or dynamic is a runtime property. Lifts in Phase 8 with the dictionary-mode escape |
| STA2005 | both | runtime | {builtin operation}; builtins cannot throw yet / Unicode case mapping is not yet supported | **The fourth runtime-emitted diagnostic**, for a Task 4.2 builtin that meets machinery that has not landed. `toUpperCase`/`toLowerCase` on any code unit above 0x7F: real case mapping needs the Unicode tables that arrive with vendored libunicode (Task 4.3), and an ASCII-only mapping would be silently wrong for exactly the characters that made the string non-ASCII. Precedent: STA2002, loudly unimplemented beats wrong. (`String.prototype.repeat`/`padStart`/`padEnd` used to abort here too; since plan-notes 200 they join the pending-check protocol and throw a catchable `RangeError` matching Node — see step 2a(c).) |
| STA2006 | both | runtime | calling a non-function at {file}:{line} | **The fifth runtime-emitted diagnostic.** Raised by `jsrt_call_at` when the callee is not a closure. The emitter bakes `file:line` in as a string literal (column is not on `Span` by design — docs/HIR.md BoundaryCheck). Builtin-internal calls still go through `jsrt_call`, which keeps the unlocated `TypeError: callee is not a function`. Until Phase 5 step 11 gives the runtime a catch around user code, the trap is fatal rather than a catchable `TypeError`. Arity padding is `jsrt_arg` (missing arguments are `undefined`); A dynamic method call (`o.m(x)` on an Unknown receiver) loads the name through the shape table (`jsrt_get_prop`, which finds hidden `#method:` slots and descriptor method tables, not just fields) and passes the receiver as argument zero when -- and only when -- the loaded closure declares one (`has_receiver`, docs/VALUE.md §4.16); a plain closure is called with user arguments only |
| STA2007 | both | runtime | a statically-shaped object cannot lose a property; planned for Phase 8 / an array element cannot be deleted; planned for Phase 5 (array holes) | **The sixth runtime-emitted diagnostic**, and STA2004's mirror image: that one is a layout asked to GROW, this one is a layout asked to SHRINK, and both are the same absent encoding — a `JSRTClass` descriptor lists its fields at fixed offsets and has no way to spell one as missing. Raised by `jsrt_delete` when an Unknown receiver turns out at run time to be a fixed-shape object holding the key, or an array whose element would become a HOLE (the gap `gateArrayLiteral` already names for `[1, , 3]`, and STA2002 for a sparse write). Where the receiver's type is known the gate refuses first, with STA1108 in ts mode and STA1205 in js. **One fixed-shape delete does have a right answer and takes it instead:** a FROZEN property is non-configurable, so `delete` on it raises Node's `TypeError` rather than aborting — the removal that cannot be represented is also the removal the spec forbids. Lifts with the same Phase 8 dictionary-mode escape STA2004 names |
| STA2008 | both | runtime | Array.prototype.{method} called on an incompatible receiver / Cannot read properties of {null\|undefined} (reading '{method}') | **The seventh runtime-emitted diagnostic, and the first catchable one in this range**: unlike STA2001/STA2002/STA2004/STA2006, which abort, this one throws a `TypeError` through the pending cell, so user `try`/`catch` observes it. Raised by `jsrt_require_array` when an Array.prototype entry receives a statically-array-typed value that is not an array at run time (`var` hoisting hands the op `undefined`; a lying annotation hands it anything) — the §0.2 boundary the dynamic representation exists to catch (plan.md §8 step 20). A nullish receiver throws Node's member-access wording, which is where Node throws for the same program; any other receiver throws the incompatible-receiver `TypeError` (Node's message names receiver source text, which compiled code no longer has). Reads and writes do not need a code of their own: a lying receiver degrades to the dynamic property path, which already throws Node's wording for nullish and misses to `undefined` otherwise |
| STA2009 | both | runtime | {value} is not iterable | **The eighth runtime-emitted diagnostic, catchable like STA2008.** Raised by `jsrt_require_array_iterable` when a `for`-`of` over a statically-array-typed iterable meets a non-array at run time — the sync walk reads the array header directly instead of going through an entry, so the check is its own statement before the loop (plan.md §8 step 20). Node-exact for a nullish receiver; best-effort past it for STA2008's source-text reason. Only the array kind takes this path; every other static kind has its own walk and its own step |
...
| STA4045 | both | internal | for-of iterable has type '{t}', which is not an array, string, Map, Set or iterator | Verifier. Separate from `STA4044` because the emitted shape differs: for-of over an array, a string, a Map or a Set compiles to a specialized loop (docs/VALUE.md §4.13); a user iterable is a MethodCall of `[Symbol.iterator]()` whose result is HIR `iterator` and walks the same boxed-iterator path |
...
| STA4058 | — | — | *(retired)* | Was "dynamic property access on a non-object value". Retired in Phase 5 step 4: `jsrt_get_prop`/`jsrt_set_prop`/`jsrt_has_prop` now give each receiver a deliberate path (nullish → TypeError, fixed object → class-descriptor walk, string `"length"` → length, other primitives → `undefined` / TypeError on set). Never reuse this number |
...
| STA4082 | both | internal | array op {op} takes an array receiver / takes {n} arguments / must produce {t} | Verifier. The `ARRAY_OPS` counterpart of STA4081: array-typed receiver, post-padding arity, and the table's result type — where `self` is the receiver's own array type (`slice`/`concat`/`fill`/`reverse`) and `element` is Unknown by the IndexAccess rule, so only the concrete result kinds are pinned. The arity is EXACT except for the variadic ops (plan.md §8 step 19): `push`/`unshift`/`splice`/`concat` hold any count from the table's minimum up, and `lastIndexOf` holds one or two — the emitter picks the runtime entry point by count. Argument types stay deliberately unchecked, for STA4081's reason |
...
| STA4094 | both | internal | Promise.prototype.{m} on '{t}', not a promise / results in '{t}', not a promise / new Promise results in '{t}', not a promise | Verifier. The instance methods and `new Promise` answer a promise unconditionally; a wrong receiver kind at `.then` is a lowering/gate disagreement |
| STA4095 | both | internal | new {Ctor} results in '{t}', not the {Ctor} layout | Verifier. The standard Error constructors. What is pinned is the LAYOUT, not merely the kind: `jsrt_error_new` writes slot 0 (`name`) and slot 1 (`message`) by index, and every `e.message` downstream resolves against the same two fields, so a type that disagreed with `runtime/src/jsrt_error.c` would be a wrong SLOT rather than a wrong answer. Comparing against `errorHType` checks the class name, the field order and the base chain at once, which is the only place the emitter's and the runtime's definitions can drift (plan.md §8 step 2a(c)) |
| STA4096 | both | internal | reference-error must have type 'unknown', got '{t}' / reference-error must carry the name it failed to resolve | Verifier. Reading a name nothing declares. `unknown` is the CONTRACT here rather than a fallback: the node stands where a value of an unknowable type would have been, and boundary insertion reads that type to decide whether a check is owed — typing it `undefined` (the C value the emitter hands back so the expression has one) would let `const n: number = undeclared` through as a static assignment of the wrong type instead of a boundary. The name is checked non-empty because it is the whole payload: the emitter spells it into `jsrt_reference_error("x")` and it is what makes the message match Node (plan.md §8 step 2a(c)) |
| STA4097 | both | internal | delete result must be boolean, got '{t}' | Verifier. The `delete` operator's answer. A `delete-prop` node carries a `target` and a `key`, either of which could plausibly have supplied its type by mistake, but the operator has exactly one result type in the language — the boolean saying whether the key is gone — and never the deleted value's. Pinning it here is what entitles the emitter to wrap `jsrt_delete` in `jsrt_bool` unconditionally, and const-fold/DCE to treat the node as boolean-valued (plan.md §8 step 2a(c)) |
| STA4098 | both | internal | extern call '{f}' takes {n} arguments, not {m} / argument {i} has type '{t}', not '{want}' / results in '{t}', not '{want}' / uses the {conv} convention on a '{ret}' return | Verifier. The four claims the `extern-call` direct C call rests on, and the C compiler cannot check any of them across the boundary: the arity is exact (the gate refuses a mismatch, so a wrong count is a lowering bug, never a source property), each argument unboxes to its declared ABI kind, the boxed result is the return kind's HType, and the `@statorError` convention reads what the return carries (plan.md §10 Task 7.1 steps 4–5) |
| STA4099 | both | internal | String.{m} must return string, got '{t}' | Verifier. The two claims a `string-static` node rests on: the method is one of `STRING_STATICS` (today only `fromCharCode`), and the answer is a `string` by construction. Any argument count lands — the node is variadic and the emitter passes the count — and argument types stay deliberately unchecked: the runtime coerces each code with ToNumber then ToUint16 (plan.md §8 step 19) |
| STA4100 | both | internal | function length must have type 'number', got '{t}' / function length needs a function operand, got '{t}' | Verifier. The two claims a `function-length` node rests on: the answer is a `number` by construction, and the operand is statically function-typed (`jsrt_closure_arity` reads the closure without asking). An `Unknown`-typed receiver never reaches this node -- it takes the dynamic path through `jsrt_get_prop` -- so a non-function operand here is a lowering bug (plan.md §8 step 21b) |
| STA4071 | both | internal | Iterator.{m} reached the lowering on a non-generator receiver | Lowering invariant: the gate admits `return`/`throw` only on a checker-typed `Generator`. Runtime receivers are validated independently and throw a catchable `TypeError` before casting; a lying JS annotation must not turn this compiler invariant into a runtime abort or invalid struct read (plan-notes 200) |
| STA4072 | both | internal | internal error: {message} — this is a compiler bug; report it with the input that triggered it | **The CLI's catch-all** (`src/cli/main.ts`), raised for any exception that is neither a `StatorError` nor a `BuildError`: the last line of AGENTS.md's "a thrown exception reaching the CLI is a compiler bug", and the reason no input can reach a user as a raw Node stack trace. It sits in the lowering's second band for room — the `STA4001` precedent, where the range is chosen by availability and the row names the raiser. First real case: upstream TypeScript's checker recurses without a depth guard and overflows the JS stack inside `getSemanticDiagnostics` on `var yield` plus a generator method whose computed key is `[yield]` (Test262 `test/language/expressions/object/method-definition/generator-prop-name-yield-expr.js`; plain `tsc` 6.0.3 dies on the same file). Stator cannot fix upstream and must not pretend the input was fine either (plan-notes 213) |
| STA4073 | both | internal | the #brand-in-object test names no class the gate accepted | Lowering. The gate admits `#x in o` only when the private name resolves to a declaring class, and the lowering desugars it to `instanceof` against that class — so an unresolvable brand here means the gate and the lowering disagree about which class a brand means (plan.md §8 step 12(d)) |
...
- **STA2010–STA2999**: Lowering and boundary errors (Phase 2 onward)
- **STA3002–STA3999**: Module graph errors (Phase 2+)
- **STA4038–STA4039, STA4074–STA4079, STA4101–STA4999**: Internal errors (may grow as invariants are formalized). STA4002–STA4020, STA4022–STA4029 and STA4040–STA4059 are the HIR verifier's; STA4030–STA4037 and STA4060–STA4079 are the lowering's. The gap between the verifier's first two ranges was deliberate room for each to grow without renumbering.
  The verifier's first band is **full** (STA4029 was the last of it) and its second band, **STA4040–STA4059**, is now full too: STA4040–STA4057 and STA4059 are the verifier's, STA4058 sat in the band as a runtime panic and is now retired (Phase 5 step 4); the number stays listed so it is never reused. The verifier's third band is **STA4080–STA4099**, of which STA4080–STA4099 are taken (STA4084, STA4091 and STA4093 raised by the runtime, STA4058-style). STA4094 is the Promise instance/construct verifier, STA4095 the Error-constructor layout check, STA4096 the undeclared-name read, STA4097 the `delete` result type, STA4098 the extern-call direct-call claims and STA4099 the `String` namespace call (plan.md §8 step 19). STA4100 is the function-length verifier (plan.md §8 step 21b). The lowering's first band is also effectively full — STA4038–STA4039 are its only remaining room, and `STA4037` is retired rather than reusable — so it has opened a second band, **STA4060–STA4079**, of which STA4060–STA4071 are taken and STA4072 is the CLI's catch-all (the `STA4001` precedent: band by availability, raiser named in the row). Each is a new band, not a renumbering of an old one, which is why the free ranges above are discontiguous.
...
| STA4058 | dynamic property access on a non-object value | The shape-table entry points used to panic on anything that was not a `JSRTDynObject`. Phase 5 step 4 gave each receiver a deliberate path, so the panic is unreachable. |
