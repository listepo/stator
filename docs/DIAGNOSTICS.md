...
| STA4045 | both | internal | for-of iterable has type '{t}', which is not an array, string, Map or Set | Verifier. Separate from `STA4044` because the emitted shape differs: for-of over an array, a string, a Map or a Set compiles to a specialized loop (docs/VALUE.md §4.13). A user iterable is still not-yet |
