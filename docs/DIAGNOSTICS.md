...
| STA1210 | both | not-yet | Date is not yet supported; planned for Phase 4 (builtins) | 4 | Stays Phase 4, but Task 4.2's builtin list did not actually name `Date` -- it was owned by nobody until 2026-09-01, when Task 4.2 was edited to claim it (plan-notes 116). `Date.now()` and zero-argument `new Date()` fall under that task's determinism carve-out: they cannot match Node byte-for-byte and land with a different proof |
