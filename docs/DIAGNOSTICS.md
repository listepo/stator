...
| STA4045 | both | internal | for-of iterable has type '{t}', which is not an array or string | Verifier. Separate from `STA4044` because the emitted shape differs: for-of over an array or a string compiles to a counted loop (docs/VALUE.md §4.13). Map/Set/user iterables are still not-yet |
