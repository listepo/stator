...
| STA1210 | both | not-yet | {member} is not yet supported; planned for Phase 4 (builtins) | 4 | `Date`'s RESIDUE code, the shape `STA1211` has for RegExp: it names one MEMBER at a time rather than the class. Slice A landed 2026-09-01 (plan-notes 130) and slice B the same day (plan-notes 133), which leaves exactly the ICU-dependent string forms: `toString` and `toTimeString`, whose output appends the host zone's LONG display name (`(Central European Summer Time)`) that Node reads from ICU and libc's `%Z` cannot produce, and the three `toLocale*`. All five join Task 4.4's feature build (the `STA1215` model). `toDateString` is NOT among them -- its output carries no zone name, so it landed with slice B. `Date.now()` and zero-argument `new Date()` are ACCEPTED, not deferred: nondeterminism is a proof problem, not an acceptance problem, and they prove through the determinism carve-out (`tests/unit/date-clock.test.ts`). Per §15's rule, when Phase 4 closes every member still under this code must be delivered or reassigned |
