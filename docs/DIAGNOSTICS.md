...
| STA4071 | both | internal | Iterator.{m} reached the lowering on a non-generator receiver / generator close on a value that is not a generator | Lowering, and the runtime from the other side of the same contract (STA4084/STA4091-style). The gate admits `return`/`throw` calls only on a receiver the checker types as the declaration-file `Generator`, and `jsrt_generator_close`/`jsrt_generator_throw` blindly read a `JSRTGenerator` -- a boxed specialized iterator arriving at either point is the gate and the lowering disagreeing about which receivers close, and its fields would be read out of a different struct (Phase 5 step 8) |
