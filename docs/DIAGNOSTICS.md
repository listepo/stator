...
| STA1211 | both | not-yet | RegExp methods are not yet supported; planned for Phase 4 (builtins) | 4 | Stays Phase 4: the blocker is an ARRAY WITH PROPERTIES (`index`, `input`, `groups` hang off a match; a dense JSRTArray cannot carry them), which is Task 4.1's hybrid extended to arrays. `test` landed with Task 4.3; `exec` and `match` are the Phase 4 residue. `String.prototype.matchAll` SPLITS OFF to Phase 5 step 8 -- it answers an iterator (plan-notes 116) |
