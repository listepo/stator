...
| STA1208 | both | not-yet | top-level await is not yet supported: a module body has no resume point to suspend into | 4 | `await` compiles to a suspend/resume pair whose resume half is a continuation the enclosing async function owns (Task 4.6). A module body is not one: there is no frame to re-enter, and `main` has already returned by the time the queue drains. Landing it means making the module body itself an async body, which is module work (Phase 7), not the event loop. In js mode a `.js` module body is parsed without the await grammar, so top-level await is refused earlier still, as `STA0012` |
