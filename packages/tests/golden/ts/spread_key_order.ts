...
// (plan.md §8 step 21a): the ts-mode twin of `spread_key_order.js`, with the reordering spelled
// as an annotation rather than JSDoc. Reads still resolve through the layout.
...
const o2: { b: number; a: string } = { a: 'A', b: 1 };
const merged = { ...o, ...o2 };
...
let calls = 0;
function make(): { q: number; w: number } {
  calls++;
  return { q: 1, w: 2 };
}
function one(): { solo: number } {
  calls++;
  return { solo: 7 };
}
const c1 = { ...make() };
...
let b: { n: number; m: number } = { n: 1, m: 2 };
...
const d: { y?: number; x?: string; z?: number } = { ...o, z: 5 };
console.log(Object.keys(d).join(','));
console.log(JSON.stringify(d));
const d2: { w?: number; y?: number; x?: string } = { w: 0, ...o };
...
const q: { y: number; x: string } = { x: 't', y: 3 };
...
function* g(): Generator<number, string, number> {
  const a = { ...q, v: yield 1 };
