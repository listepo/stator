...
function ownershipComment(fn: MappedFunction): string {
  const parts: string[] = [];
  const borrows = fn.params.filter((p) => p.ownership !== 'value');
  if (borrows.length > 0) {
    parts.push(
      borrows
...
export function generate(model: HeaderModel): GenResult {
  const takenNames = new Set<string>(['CString', 'CStringOwned']);
...
/** The whole `.d.ts`, deterministic: header legend, brands, declarations, REFUSED block. */
export function renderDts(result: GenResult): string {
  const out: string[] = [];
  ...
  for (const fn of result.functions) {
    ...
    out.push(`declare function ${fn.tsName}(${params}): ${fn.ret};`);
    out.push('');
  }
