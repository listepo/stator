...
45. **[D3][P0] Boundary widening stops at the call edge** (plan-notes 262). Step 44 fixed the call-argument edge (Unknown/dynamic values into fixed params widen through the 2322/2454 channel), but the same silent garbage (`2e-323`) survives on three sibling edges: `return JSON.parse(...)` from an object-typed function, `const x: T = <dynamic>` declarations, and assignments of dynamic values into fixed-shape bindings (repros in the wave-5 report). Each is §0.2 territory — a value the checker typed but wrong at run time. **Check:** a golden per edge matching Node byte-for-byte (dynamic route or catchable `STA2001`); no silent wrong answers; `spread_key_order` and `class_generic_base` stay green (the two regressions the first attempt hit and reverted).
