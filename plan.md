...
41. **[D3][P1] Bare generics as value** (plan-notes 261). A generic function used as a value (`const f = id` called excluded — that specializes through the alias and already lands) and `instanceof` against anything but a named class are `STA1214` in both modes where Node answers. Land the value path (specialize at the use site or route dynamic) or scope the refusal precisely; `instanceof` against a generic class stays refused (no family registry — note 261). **Check:** goldens for generic-as-value and explicit-type-args positions matching Node in both modes; decision fixtures flipped.
42. **[D2][P1] `super` as value** (plan-notes 261, pointers in wave-4 report). `const s = super.m` is `STA1214` where Node answers the function. Lower to a `method-value` node with target = receiver parameter and owner = base class, dispatch forced `direct`; bare `super` stays refused. **Check:** goldens for `super.m` tear-off + call matching Node in both modes; fixtures flipped.
43. **[D3][P2] `extends` beyond a class declaration** (plan-notes 261). Mixins and expression bases are `STA1214`; `extends K` (class alias) now reports the sharper heritage message but still refuses although alias erasure could carry it. Land alias/namespaced bases that resolve to one declaration; true mixins stay refused with the existing message. **Check:** goldens for alias-base and member-expression-base subclasses matching Node; mixin fixture still `STA1214`.
44. **[D3][P1] Element/spread receiver hardening** (plan-notes 259, 261). Three adjacent loud-or-wrong behaviors: `[...m]` over `RegExpExecArray` is gate-accepted then `STA4082`; `o[kObj]` reads hit unsuppressed TS2538; js-mode fixed-param + dynamic-arg miscompiles (`2e-323`). Each is valid at run time and each is either an `STA4xxx` or silent garbage. **Check:** a golden or decision test per case — dynamic path with Node's answer or a `not-yet` naming its blocker; no `STA4xxx`, no silent wrong answers.
